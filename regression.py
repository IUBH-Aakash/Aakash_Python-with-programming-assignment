...
def minimise_loss(training_function, list_of_candidate_functions, loss_function):
    """
    It returns an IdealFunction based on a training function and a list of ideal functions
...
def find_classification(point, ideal_functions):
    ...
    :param point: a dictionay object in there is an "x" and an "y"
    ...
    for ideal_function in ideal_functions:
        ...
        # It will show the absolute distance
        distance = abs(locate_y_in_classification - point["y"])

        if (abs(distance < ideal_function.tolerance)):
            # If there are multiple classifcations possible, below procedure will handle it and will return the one with lowest distance.
